"""
玄枢 AI Engine v11.2.0 — Edge TTS 流式语音引擎
==============================================
...
class AudioPlayer:
    ...
    def __init__(self):
        # 使用 maxsize=50 限制队列容量，生产者满时阻塞实现背压控制
    ...
    def _play_wav(file_path: str):
        ...
        try:
            ...
            # M-04 修复：从 WAV header 读取精确时长，替代固定码率估算
            duration = VoiceEngine._get_wav_duration(file_path)
            time.sleep(duration)
        except ImportError:
            # 降级到 ffplay
            subprocess.run(
    ...
    def _get_wav_duration(file_path: str) -> float:
        """M-04 修复：从 WAV header 读取精确播放时长（秒）"""
        try:
            import wave
            with wave.open(file_path, 'rb') as wf:
                ...
                if rate > 0:
                    return max(frames / rate, 0.5)
                return 1.0
        except Exception:
            # 无法解析时回退到文件大小估算
            try:
                file_size = os.path.getsize(file_path)
                return max(file_size / 32000, 0.5)
            except Exception:
                return 1.0
