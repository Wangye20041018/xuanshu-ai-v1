...
Models (常驻投机解码):
  ...
  Verify:  (已退役 v11.2 — 14B 验证模型已移除，投机解码已禁用)

Sources:
  - VL models from ggml-org (HuggingFace: ggml-org/Qwen2.5-VL-7B-Instruct-GGUF)
  - 14B legacy reference: Qwen/Qwen2.5-14B-Instruct-GGUF (retained for VERIFY_MODEL dict backward compat)
"""
...
# 模型目录：config.json model_dir → 环境变量 XUANSHU_MODEL_DIR → PROJECT_ROOT/models
def _resolve_models_dir() -> Path:
    r"""从 config.json / 环境变量 / 默认路径解析模型目录（TASK-C：消除 D:\模型\ 硬编码）"""
    ...
    if config_path.exists():
        try:
            ...
            pass
    env_dir = os.environ.get("XUANSHU_MODEL_DIR", "")
    if env_dir:
        return Path(env_dir)
...
# Expected SHA256 hashes (L-04: 首次下载后自动计算并持久化到 model_hashes.json，空串表示尚未记录)
EXPECTED_HASHES = {
...
HASH_STORE_PATH = MODELS_DIR / "model_hashes.json"


def _load_expected_hashes() -> Dict[str, str]:
    """从 model_hashes.json 加载已记录的哈希（无记录或损坏时返回空）"""
    try:
        ...
        logger.warning(f"加载 model_hashes.json 失败: {e}")
    return {}


def _save_expected_hashes() -> None:
    """持久化 EXPECTED_HASHES 到 model_hashes.json，使后续校验可复用真实哈希"""
    try:
        HASH_STORE_PATH.parent.mkdir(parents=True, exist_ok=True)
        with open(HASH_STORE_PATH, "w", encoding="utf-8") as f:
            json.dump(EXPECTED_HASHES, f, ensure_ascii=False, indent=2)
    except Exception as e:
        logger.warning(f"保存 model_hashes.json 失败: {e}")


EXPECTED_HASHES.update(_load_expected_hashes())
    ...
    def download_draft(self, force: bool = False) -> Dict:
        ...
        for file_info in DRAFT_MODEL["files"]:
            ...
            try:
                ...
                # Verify (L-04: 传入已记录哈希做真实校验；下载成功自动回写并持久化)
                valid, hash_val = verify_model(Path(downloaded), EXPECTED_HASHES.get(filename, ""))
                if valid and hash_val and EXPECTED_HASHES.get(filename) != hash_val:
                    EXPECTED_HASHES[filename] = hash_val
                    _save_expected_hashes()
                results.append({
                    "filename": filename,
                    "status": "downloaded",
                    "path": downloaded,
