...
# llama-server / python / ollama 已显式加入白名单，防止推理进程被误杀
...
# C-13 修复：白名单小写集合缓存为模块级常量，避免每次循环重建
_WHITELIST_LOWER = frozenset(w.lower() for w in WHITELIST_NAMES)
    ...
    def pre_clean(self, target_free_gb: float = 4.0) -> float:
        ...
        for proc in psutil.process_iter(["pid", "name", "memory_info", "cmdline"]):
            try:
                ...
                # Skip whitelisted (C-13 修复：使用缓存的模块级常量集合)
                if name in _WHITELIST_LOWER:
                    continue
        ...
        # C-13 修复：自动清理前明确告警日志，帮助用户/开发者追溯
        logger.warning(
            f"[C-13] 内存守护正在自动清理非关键进程，目标释放 {target_free_gb} GB；"
            f"仅终止 KILLABLE_PREFIXES 白名单前缀且未在白名单内的进程"
        )
